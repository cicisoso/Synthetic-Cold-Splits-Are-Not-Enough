...
case "$MODEL" in
  dtilm)
    ...
    OUT_DIR="results/recent_dtilm_ki_multiseed"
    LOG_PATH="logs/dtilm_ki_fast.log"
    ;;
  hyperpcm)
    ENCODER_BATCH_SIZE=2
    OUT_DIR="results/recent_hyperpcm_ki_multiseed"
    LOG_PATH="logs/hyper_ki_fast.log"
    ;;
  *)
...
for seed in "${SEEDS[@]}"; do
  run_if_missing \
    "${OUT_DIR}/BindingDB_Ki_unseen_target_${MODEL}_seed${seed}.json" \
...
echo "{\"status\":\"ki ${MODEL} complete\"}" | tee -a "$LOG_PATH"
